...
#define ASSERT_INIT                                                            \
  C3D_ASSERT_MSG(GetInitialized(), "Logger was used before it was "            \
                                   "initialized!")
...
#define DEBUG_LOG(format, ...)                                                 \
  C3D::Logger::Debug("[{}:{}] - " format, __FUNCTION__, __LINE__,              \
                     ##__VA_ARGS__);
#else
#define DEBUG_LOG(format, ...)
#endif
...
#ifdef C3D_LOG_TRACE
#define TRACE(format, ...) C3D::Logger::Trace(""[{}:{}] - " format, __FUNCTION__, __LINE__, ##__VA_ARGS__);
#else
#define TRACE(format, ...)
#endif
...
#define ERROR_LOG(format, ...)                                                 \
  C3D::Logger::Error("[{}:{}] - " format, __FUNCTION__, __LINE__,              \
                     ##__VA_ARGS__);
#else
#define ERROR_LOG(format, ...)
#endif
...
#define WARN_LOG(format, ...)                                                  \
  C3D::Logger::Warn("[{}:{}] - " format, __FUNCTION__, __LINE__, ##__VA_ARGS__);
#else
#define WARN_LOG(format, ...)
#endif
...
#define INFO_LOG(format, ...)                                                  \
  C3D::Logger::Info("[{}:{}] - " format, __FUNCTION__, __LINE__, ##__VA_ARGS__);
#else
#define INFO_LOG(format, ...)
#endif

#define FATAL_LOG(format, ...)                                                 \
  C3D::Logger::Fatal("[{}:{}] - " format, __FUNCTION__, __LINE__,              \
                     ##__VA_ARGS__);
} // namespace C3D
