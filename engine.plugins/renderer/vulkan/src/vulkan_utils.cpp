...
namespace C3D
{
        ...
        switch (result)
        {
            ...
            case VK_THREAD_IDLE_KHR:
                return !getExtended ? "VK_THREAD_IDLE_KHR"
                                    : "VK_THREAD_IDLE_KHR A deferred operation is not complete but there is currently "
            ...
            case VK_OPERATION_DEFERRED_KHR:
                return !getExtended ? "VK_OPERATION_DEFERRED_KHR"
                                    : "VK_OPERATION_DEFERRED_KHR A deferred operation was requested and at least some "
                                      "of the work was deferred.";
            case VK_OPERATION_NOT_DEFERRED_KHR:
            ...
            case VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT:
                return !getExtended ? "VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT"
                                    : "VK_ERROR_FULL_SCREEN_EXCLUSIVE_MODE_LOST_EXT An operation on a swapchain "
                                      "created with VK_FULL_SCREEN_EXCLUSIVE_APPLICATION_CONTROLLED_EXT failed as it "
    ...
    u32 VulkanUtils::GetAvailableGPUMemoryInMB(VkPhysicalDeviceMemoryProperties properties)
    {
        ...
        for (u32 i = 0; i < properties.memoryHeapCount; i++)
        {
            ...
            {
                gpuMemory += static_cast<f32>(heap.size) / 1024.0f / 1024.0f;
            }
        }
