...
void main()
{
    ...
    vec3 center = RotateVecByQuat(mesh.center, drawData.orientation) * drawData.scale + drawData.position;
    ...
    if (visible && (!LATE || (cullData.clusterOcclusionCullingEnabled == 1 && TASK_CULL == 1) || drawVisibility[di] == 0 || cullData.postPass != 0))
    {
        uint lodIndex = 0;
        ...
        if (TASK)
        {
            ...
            // Drop calls on overflow; This limits us to ~4M visible draws or 32Billion visible triangles, whichever is larger
