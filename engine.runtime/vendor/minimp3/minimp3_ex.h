...
/*
   ...
   public domain worldwide. This software is distributed without any warranty. See <http://creativecommons.org/publicdomain/zero/1.0/>.
*/
#include <stddef.h>
...
#define MP3D_SEEK_TO_SAMPLE \
    1 /* mp3dec_ex_seek precisely seeks to sample using index (created during duration calculation scan or when mp3dec_ex_seek called) */
#define MP3D_DO_NOT_SCAN                                                                                                                   \
    2 /* do not scan whole stream for duration if vbrtag not found, mp3dec_ex_t::samples will be filled only if mp3dec_ex_t::vbr_tag_found \
         == 1 */
...
static void mp3dec_skip_id3v1(const uint8_t *buf, size_t *pbuf_size)
{
    ...
    {
        buf_size -= 128;
        if (buf_size >= 227 && !memcmp(buf + buf_size - 227, "TAG+", 4)) buf_size -= 227;
    }
#endif
    ...
    if (buf_size > 32 && !memcmp(buf + buf_size - 32, "APETAGEX", 8))
    {
        ...
        if (buf_size >= tag_size) buf_size -= tag_size;
    }
#endif
    *pbuf_size = buf_size;
}
...
#define MINIMP3_ID3_DETECT_SIZE 10
        ...
        !((buf[5] & 15) || (buf[6] & 0x80) || (buf[7] & 0x80) || (buf[8] & 0x80) || (buf[9] & 0x80)))
    {
        ...
        if ((buf[5] & 16)) id3v2size += 10; /* footer */
        return id3v2size;
    }
#endif
    return 0;
...
static void mp3dec_skip_id3(const uint8_t **pbuf, size_t *pbuf_size)
{
    ...
    if (id3v2size)
    {
        if (id3v2size >= buf_size) id3v2size = buf_size;
        buf += id3v2size;
        buf_size -= id3v2size;
    }
...
#define VBR_SCALE_FLAG 8
    ...
    if (memcmp(g_xing_tag, tag, 4) && memcmp(g_info_tag, tag, 4)) return 0;
    int flags = tag[7];
    if (!((flags & FRAMES_FLAG))) return -1;
    tag += 8;
    *frames = (uint32_t)(tag[0] << 24) | (tag[1] << 16) | (tag[2] << 8) | tag[3];
    tag += 4;
    if (flags & BYTES_FLAG) tag += 4;
    if (flags & TOC_FLAG) tag += 100;
    if (flags & VBR_SCALE_FLAG) tag += 4;
    *delay = *padding = 0;
    if (*tag)
    { /* extension, LAME, Lavc, etc. Should be the same structure. */
        tag += 21;
        if (tag - frame + 14 >= frame_size) return 0;
...
int mp3dec_detect_buf(const uint8_t *buf, size_t buf_size) { return mp3dec_detect_cb(0, (uint8_t *)buf, buf_size); }

int mp3dec_detect_cb(mp3dec_io_t *io, uint8_t *buf, size_t buf_size)
{
    if (!buf || (size_t)-1 == buf_size || (io && buf_size < MINIMP3_BUF_SIZE)) return MP3D_E_PARAM;
    size_t filled = buf_size;
    if (io)
    {
        if (io->seek(0, io->seek_data)) return MP3D_E_IOERROR;
        filled = io->read(buf, MINIMP3_ID3_DETECT_SIZE, io->read_data);
        if (filled > MINIMP3_ID3_DETECT_SIZE) return MP3D_E_IOERROR;
    }
    if (filled < MINIMP3_ID3_DETECT_SIZE) return MP3D_E_USER; /* too small, can't be mp3/mpa */
    if (mp3dec_skip_id3v2(buf, filled)) return 0;             /* id3v2 tag is enough evidence */
    if (io)
    {
        size_t readed = io->read(buf + MINIMP3_ID3_DETECT_SIZE, buf_size - MINIMP3_ID3_DETECT_SIZE, io->read_data);
        if (readed > (buf_size - MINIMP3_ID3_DETECT_SIZE)) return MP3D_E_IOERROR;
        filled += readed;
        if (filled < MINIMP3_BUF_SIZE) mp3dec_skip_id3v1(buf, &filled);
    }
    else
    {
        mp3dec_skip_id3v1(buf, &filled);
        if (filled > MINIMP3_BUF_SIZE) filled = MINIMP3_BUF_SIZE;
    }
    int free_format_bytes, frame_size;
    mp3d_find_frame(buf, filled, &free_format_bytes, &frame_size);
    if (frame_size) return 0; /* MAX_FRAME_SYNC_MATCHES consecutive frames found */
    return MP3D_E_USER;
}
...
int mp3dec_load_cb(mp3dec_t *dec, mp3dec_io_t *io, uint8_t *buf, size_t buf_size, mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data)
{
    ...
    if (io)
    {
        ...
        filled = io->read(buf, MINIMP3_ID3_DETECT_SIZE, io->read_data);
        if (filled > MINIMP3_ID3_DETECT_SIZE) return MP3D_E_IOERROR;
        if (MINIMP3_ID3_DETECT_SIZE != filled) return 0;
        size_t id3v2size = mp3dec_skip_id3v2(buf, filled);
        if (id3v2size)
        {
            if (io->seek(id3v2size, io->seek_data)) return MP3D_E_IOERROR;
            filled = io->read(buf, buf_size, io->read_data);
            if (filled > buf_size) return MP3D_E_IOERROR;
        }
        else
        {
            size_t readed = io->read(buf + MINIMP3_ID3_DETECT_SIZE, buf_size - MINIMP3_ID3_DETECT_SIZE, io->read_data);
    ...
    do
    {
        ...
        if (io)
        {
            ...
            { /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
                ...
                if (readed > (buf_size - filled)) return MP3D_E_IOERROR;
                if (readed != (buf_size - filled)) eof = 1;
                filled += readed;
                if (eof) mp3dec_skip_id3v1(buf, &filled);
            }
        ...
        if (ret > 0)
        {
            ...
            if (detected_samples >= (uint64_t)to_skip) detected_samples -= to_skip;
            if (padding > 0 && detected_samples >= (uint64_t)padding) detected_samples -= padding;
            if (!detected_samples) return 0;
        }
        if (ret)
        {
            if (io)
    ...
    if (!info->buffer) return MP3D_E_MEMORY;
    /* save info */
    ...
    do
    {
        if ((allocated - info->samples * sizeof(mp3d_sample_t)) < MINIMP3_MAX_SAMPLES_PER_FRAME * sizeof(mp3d_sample_t))
        {
            ...
            if (!alloc_buf) return MP3D_E_MEMORY;
    ...
    if (detected_samples && info->samples > detected_samples) info->samples = detected_samples; /* cut padding */
    ...
    if (allocated != info->samples * sizeof(mp3d_sample_t))
    {
        ...
        if (!alloc_buf && info->samples) return MP3D_E_MEMORY;
        info->buffer = alloc_buf;
    }
    if (frames) info->avg_bitrate_kbps = avg_bitrate_kbps / frames;
    return ret;
}
...
int mp3dec_iterate_buf(const uint8_t *buf, size_t buf_size, MP3D_ITERATE_CB callback, void *user_data)
{
    ...
    if (!buf || (size_t)-1 == buf_size || !callback) return MP3D_E_PARAM;
    /* skip id3 */
    ...
    do
    {
        ...
        if (callback)
        {
            if ((ret = callback(user_data, hdr, frame_size, free_format_bytes, buf_size, hdr - orig_buf, &frame_info))) return ret;
        }
        buf += frame_size;
        buf_size -= frame_size;
    } while (1);
...
int mp3dec_iterate_cb(mp3dec_io_t *io, uint8_t *buf, size_t buf_size, MP3D_ITERATE_CB callback, void *user_data)
{
    if (!io || !buf || (size_t)-1 == buf_size || buf_size < MINIMP3_BUF_SIZE || !callback) return MP3D_E_PARAM;
    ...
    do
    {
        ...
        if (callback)
        {
            if ((ret = callback(user_data, hdr, frame_size, free_format_bytes, filled - consumed, readed, &frame_info))) return ret;
        }
        readed += frame_size;
...
static int mp3dec_load_index(void *user_data, const uint8_t *frame, int frame_size, int free_format_bytes, size_t buf_size, uint64_t offset,
                             mp3dec_frame_info_t *info)
{
    ...
    { /* detect VBR tag and try to avoid full scan */
        ...
        if (3 == dec->info.layer)
        {
            ...
            if (ret) dec->start_offset = dec->offset = offset + frame_size;
            if (ret > 0)
            {
                ...
                if (dec->samples >= (uint64_t)dec->start_delay) dec->samples -= dec->start_delay;
                if (padding > 0 && dec->samples >= (uint64_t)padding) dec->samples -= padding;
    ...
    if (dec->flags & MP3D_DO_NOT_SCAN) return MP3D_E_USER;
...
int mp3dec_ex_open_buf(mp3dec_ex_t *dec, const uint8_t *buf, size_t buf_size, int flags)
{
    if (!dec || !buf || (size_t)-1 == buf_size || (flags & (~MP3D_FLAGS_MASK))) return MP3D_E_PARAM;
    ...
    if (ret && MP3D_E_USER != ret) return ret;
    mp3dec_init(&dec->mp3d);
...
static size_t mp3dec_idx_binary_search(mp3dec_index_t *idx, uint64_t position)
{
    ...
    while (start <= end)
    {
        ...
        { /* move left side. */
            if (idx->frames[mid].sample == position) return mid;
            end = mid - 1;
        }
        else
        { /* move to right side */
...
int mp3dec_ex_seek(mp3dec_ex_t *dec, uint64_t position)
{
    ...
    { /* no index created yet (vbr tag used to calculate track length or MP3D_DO_NOT_SCAN open flag used) */
        ...
        if (dec->io)
        {
            if (dec->io->seek(dec->start_offset, dec->io->seek_data)) return MP3D_E_IOERROR;
        ...
        for (i = 0; i < dec->index.num_frames; i++) dec->index.frames[i].offset += dec->start_offset;
        dec->samples = dec->detected_samples;
    }
    if (!dec->index.frames) goto seek_zero; /* no frames in file - seek to zero */
#ifdef MINIMP3_SEEK_IDX_LINEAR_SEARCH
    for (i = 0; i < dec->index.num_frames; i++)
    {
        if (dec->index.frames[i].sample >= position) break;
    }
#else
    ...
    if (i)
    {
        ...
        if (3 == dec->info.layer)
        {
            ...
            { /* make sure bit-reservoir is filled when we start decoding */
                ...
                {
                    hdr = dec->file.buffer;
                    if (dec->io->seek(dec->index.frames[i - 1].offset, dec->io->seek_data)) return MP3D_E_IOERROR;
                    size_t readed = dec->io->read((uint8_t *)hdr, HDR_SIZE, dec->io->read_data);
                    if (readed != HDR_SIZE) return MP3D_E_IOERROR;
                    ...
                    if (readed != (size_t)(frame_size - HDR_SIZE)) return MP3D_E_IOERROR;
                ...
                if (HDR_IS_CRC(hdr)) get_bits(bs, 16);
                i--;
                if (L3_read_side_info(bs, gr_info, hdr) < 0) break; /* frame not decodable, we can start from here */
    ...
    { /* skip not decodable first frames */
        ...
        {
            hdr = dec->file.buffer;
            if (dec->io->seek(dec->index.frames[i].offset, dec->io->seek_data)) return MP3D_E_IOERROR;
            size_t readed = dec->io->read((uint8_t *)hdr, HDR_SIZE, dec->io->read_data);
            if (readed != HDR_SIZE) return MP3D_E_IOERROR;
        }
        else
...
do_exit:
    if (dec->io)
    {
        if (dec->io->seek(dec->offset, dec->io->seek_data)) return MP3D_E_IOERROR;
    }
    dec->buffer_samples  = 0;
...
size_t mp3dec_ex_read_frame(mp3dec_ex_t *dec, mp3d_sample_t **buf, mp3dec_frame_info_t *frame_info, size_t max_samples)
{
    ...
    {
        if (dec) dec->last_error = MP3D_E_PARAM;
        return 0;
    }
    if (dec->detected_samples && dec->cur_sample >= dec->detected_samples) return 0; /* at end of stream */
    if (dec->last_error) return 0;                                                   /* error eof state, seek can reset it */
    *buf                = NULL;
    ...
    while (dec->buffer_consumed == dec->buffer_samples)
    {
        ...
        if (dec->io)
        {
            ...
            { /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
                ...
                if (readed != (dec->file.size - dec->input_filled)) eof = 1;
                dec->input_filled += readed;
                if (eof) mp3dec_skip_id3v1((uint8_t *)dec->file.buffer, &dec->input_filled);
            }
    ...
    { /* count decoded samples to properly cut padding */
        if (dec->cur_sample + out_samples >= dec->detected_samples) out_samples = dec->detected_samples - dec->cur_sample;
    }
...
int mp3dec_ex_open_cb(mp3dec_ex_t *dec, mp3dec_io_t *io, int flags)
{
    if (!dec || !io || (flags & (~MP3D_FLAGS_MASK))) return MP3D_E_PARAM;
    memset(dec, 0, sizeof(*dec));
#ifdef MINIMP3_HAVE_RING
    int ret;
    if (ret = mp3dec_open_ring(&dec->file, MINIMP3_IO_SIZE)) return ret;
#else
    dec->file.size   = MINIMP3_IO_SIZE;
    dec->file.buffer = (const uint8_t *)malloc(dec->file.size);
    if (!dec->file.buffer) return MP3D_E_MEMORY;
#endif
    dec->flags = flags;
    dec->io    = io;
    mp3dec_init(&dec->mp3d);
...
{
    if (map_info->buffer && MAP_FAILED != map_info->buffer) munmap((void *)map_info->buffer, map_info->size);
    map_info->buffer = 0;
    map_info->size   = 0;
}
...
{
    if (!file_name) return MP3D_E_PARAM;
    int file;
    struct stat st;
    memset(map_info, 0, sizeof(*map_info));
retry_open:
    file = open(file_name, O_RDONLY);
    if (file < 0 && (errno == EAGAIN || errno == EINTR)) goto retry_open;
...
retry_mmap:
    ...
    if (MAP_FAILED == map_info->buffer && (errno == EAGAIN || errno == EINTR)) goto retry_mmap;
    close(file);
    if (MAP_FAILED == map_info->buffer) return MP3D_E_IOERROR;
    return 0;
}
...
static void mp3dec_close_ring(mp3dec_map_info_t *map_info)
{
    ...
    if (map_info->buffer && MAP_FAILED != map_info->buffer) munmap((void *)map_info->buffer, map_info->size * 2);
#else
    if (map_info->buffer)
    {
...
static int mp3dec_open_ring(mp3dec_map_info_t *map_info, size_t size)
{
    ...
    if (memfd < 0) return MP3D_E_MEMORY;

retry_ftruncate:
    res = ftruncate(memfd, map_info->size);
    if (res && (errno == EAGAIN || errno == EINTR)) goto retry_ftruncate;
    if (res) goto error;

retry_mmap:
    map_info->buffer = (const uint8_t *)mmap(NULL, map_info->size * 2, PROT_NONE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    if (MAP_FAILED == map_info->buffer && (errno == EAGAIN || errno == EINTR)) goto retry_mmap;
    if (MAP_FAILED == map_info->buffer || !map_info->buffer) goto error;
retry_mmap2:
    buffer = mmap((void *)map_info->buffer, map_info->size, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_SHARED, memfd, 0);
    if (MAP_FAILED == map_info->buffer && (errno == EAGAIN || errno == EINTR)) goto retry_mmap2;
    if (MAP_FAILED == map_info->buffer || buffer != (void *)map_info->buffer) goto error;
retry_mmap3:
    buffer = mmap((void *)map_info->buffer + map_info->size, map_info->size, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_SHARED, memfd, 0);
    if (MAP_FAILED == map_info->buffer && (errno == EAGAIN || errno == EINTR)) goto retry_mmap3;
    if (MAP_FAILED == map_info->buffer || buffer != (void *)(map_info->buffer + map_info->size)) goto error;
...
retry_mmap:
    ...
    if (MAP_FAILED == map_info->buffer && (errno == EAGAIN || errno == EINTR)) goto retry_mmap;
    if (MAP_FAILED == map_info->buffer) goto error;
    if (map_info->buffer != shmat(memfd, map_info->buffer, 0)) goto error;
    if ((map_info->buffer + map_info->size) != shmat(memfd, map_info->buffer + map_info->size, 0)) goto error;
    if (shmctl(memfd, IPC_RMID, NULL) < 0) return MP3D_E_MEMORY;
    return 0;
error:
...
static void mp3dec_close_file(mp3dec_map_info_t *map_info)
{
    if (map_info->buffer) UnmapViewOfFile(map_info->buffer);
    map_info->buffer = 0;
    map_info->size   = 0;
}
...
static int mp3dec_open_file_h(HANDLE file, mp3dec_map_info_t *map_info)
{
    ...
    if (s.LowPart == INVALID_FILE_SIZE && GetLastError() != NO_ERROR) goto error;
...
static int mp3dec_open_file(const char *file_name, mp3dec_map_info_t *map_info)
{
    ...
    if (INVALID_HANDLE_VALUE == file) return MP3D_E_IOERROR;
...
static void mp3dec_close_file(mp3dec_map_info_t *map_info)
{
    if (map_info->buffer) free((void *)map_info->buffer);
    map_info->buffer = 0;
    map_info->size   = 0;
}
...
static int mp3dec_open_file(const char *file_name, mp3dec_map_info_t *map_info)
{
    ...
    if (fread((void *)map_info->buffer, 1, map_info->size, file) != map_info->size) goto error;
    fclose(file);
    return 0;
error:
    mp3dec_close_file(map_info);
...
int mp3dec_detect(const char *file_name)
{
    ...
    if ((ret = mp3dec_open_file(file_name, &map_info))) return ret;
...
int mp3dec_ex_open(mp3dec_ex_t *dec, const char *file_name, int flags)
{
    int ret;
    if (!dec) return MP3D_E_PARAM;
    if ((ret = mp3dec_open_file(file_name, &dec->file))) return ret;
...
{
#ifdef MINIMP3_HAVE_RING
    if (dec->io) mp3dec_close_ring(&dec->file);
#else
    if (dec->io && dec->file.buffer) free((void *)dec->file.buffer);
#endif
    if (dec->is_file) mp3dec_close_file(&dec->file);
    if (dec->index.frames) free(dec->index.frames);
    memset(dec, 0, sizeof(*dec));
}
...
int mp3dec_detect_w(const wchar_t *file_name)
{
    int ret;
    mp3dec_map_info_t map_info;
    if ((ret = mp3dec_open_file_w(file_name, &map_info))) return ret;
    return mp3dec_detect_mapinfo(&map_info);
}
...
{
    int ret;
    if ((ret = mp3dec_open_file_w(file_name, &dec->file))) return ret;
    return mp3dec_ex_open_mapinfo(dec, flags);
}
#endif
