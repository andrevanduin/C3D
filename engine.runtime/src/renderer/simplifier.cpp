...
namespace meshopt
{
    ...
    struct EdgeAdjacency
    {
        ...
        EdgeAdjacency(size_t index_count, size_t vertex_count) : counts(vertex_count), offsets(vertex_count), data(index_count) {}
    };
    ...
    static unsigned int findWedgeEdge(const EdgeAdjacency& adjacency, const unsigned int* wedge, unsigned int a, unsigned int b)
    {
        ...
        do
        {
            if (hasEdge(adjacency, v, b)) return v;
    ...
    static void classifyVertices(unsigned char* result, unsigned int* loop, size_t vertex_count, const EdgeAdjacency& adjacency, const unsigned int* remap,
                                 const unsigned int* wedge)
    {
        for (size_t i = 0; i < vertex_count; ++i) loop[i] = ~0u;
    ...
    static void fillFaceQuadrics(Quadric* vertex_quadrics, const unsigned int* indices, size_t index_count, const Vector3* vertex_positions,
    ...
    static size_t pickEdgeCollapses(Collapse* collapses, const unsigned int* indices, size_t index_count, const unsigned int* remap,
    ...
    static void rankEdgeCollapses(Collapse* collapses, size_t collapse_count, const Vector3* vertex_positions, const Quadric* vertex_quadrics,
                                  const unsigned int* remap)
    {
    ...
    static void dumpEdgeCollapses(const Collapse* collapses, size_t collapse_count, const unsigned char* vertex_kind)
    {
        ...
        for (int k0 = 0; k0 < Kind_Count; ++k0)
            for (int k1 = 0; k1 < Kind_Count; ++k1) cerrors[k0][k1] = FLT_MAX;
        ...
        for (int k0 = 0; k0 < Kind_Count; ++k0)
            for (int k1 = 0; k1 < Kind_Count; ++k1)
                if (ckinds[k0][k1]) printf("collapses %d -> %d: %d, min error %e\n", k0, k1, int(ckinds[k0][k1]), cerrors[k0][k1]);
    }
    ...
    static void dumpLockedCollapses(const unsigned int* indices, size_t index_count, const unsigned char* vertex_kind)
    {
        ...
        for (int k0 = 0; k0 < Kind_Count; ++k0)
            for (int k1 = 0; k1 < Kind_Count; ++k1)
                if (locked_collapses[k0][k1]) printf("locked collapses %d -> %d: %d\n", k0, k1, int(locked_collapses[k0][k1]));
    }
#endif
    ...
    static size_t performEdgeCollapses(unsigned int* collapse_remap, unsigned char* collapse_locked, Quadric* vertex_quadrics, const Collapse* collapses,
                                       size_t collapse_count, const unsigned int* collapse_order, const unsigned int* remap, const unsigned int* wedge,
                                       const unsigned char* vertex_kind, size_t triangle_collapse_goal, float error_limit)
    {
        ...
        for (size_t i = 0; i < collapse_count; ++i)
        {
            ...
            if (triangle_collapses >= triangle_collapse_goal) break;
            ...
            if (collapse_locked[r0] | collapse_locked[r1]) continue;
                        ...
                        size_t vertex_positions_stride, size_t target_index_count, float target_error)
{
    ...
    if (result != indices) memcpy(result, indices, index_count * sizeof(unsigned int));
    ...
    while (result_count > target_index_count)
    {
        ...
        size_t collapses = performEdgeCollapses(collapse_remap.data, collapse_locked.data, vertex_quadrics.data, edge_collapses.data, edge_collapse_count,
                                                collapse_order.data, remap.data, wedge.data, vertex_kind.data, triangle_collapse_goal, error_limit);
