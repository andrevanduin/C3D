...
size_t meshopt_buildMeshlets(meshopt_Meshlet* destination, const unsigned int* indices, size_t index_count, size_t vertex_count, size_t max_vertices,
                             size_t max_triangles)
{
    ...
    for (size_t i = 0; i < index_count; i += 3)
    {
        ...
        if (meshlet.vertex_count + used_extra > max_vertices || meshlet.triangle_count >= max_triangles)
        {
            ...
            for (size_t j = 0; j < meshlet.vertex_count; ++j) used[meshlet.vertices[j]] = 0xff;
    ...
    if (meshlet.triangle_count) destination[offset++] = meshlet;
...
meshopt_Bounds meshopt_computeClusterBounds(const unsigned int* indices, size_t index_count, const float* vertex_positions, size_t vertex_count,
