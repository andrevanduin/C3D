...
namespace meshopt
{
    ...
    static const float kVertexScoreTableCache[1 + kCacheSizeMax] = { 0.f,    0.792f, 0.767f, 0.764f, 0.956f, 0.827f, 0.751f, 0.820f, 0.864f,
                                                                     0.738f, 0.788f, 0.642f, 0.646f, 0.165f, 0.654f, 0.545f, 0.284f };

    static const float kVertexScoreTableLive[1 + kValenceMax] = { 0.f, 0.994f, 0.721f, 0.479f, 0.423f, 0.174f, 0.080f, 0.249f, 0.056f };
    ...
    struct TriangleAdjacency
    {
        ...
        TriangleAdjacency(size_t index_count, size_t vertex_count) : counts(vertex_count), offsets(vertex_count), data(index_count) {}
    };
    ...
    static unsigned int getNextVertexDeadEnd(const unsigned int* dead_end, unsigned int& dead_end_top, unsigned int& input_cursor,
                                             const unsigned int* live_triangles, size_t vertex_count)
    {
        ...
        while (dead_end_top)
        {
            ...
            if (live_triangles[vertex] > 0) return vertex;
        }

        // input order
        while (input_cursor < vertex_count)
        {
            if (live_triangles[input_cursor] > 0) return input_cursor;
    ...
    static unsigned int getNextVertexNeighbour(const unsigned int* next_candidates_begin, const unsigned int* next_candidates_end,
                                               const unsigned int* live_triangles, const unsigned int* cache_timestamps, unsigned int timestamp,
    ...
    while (current_vertex != ~0u)
    {
        ...
        for (const unsigned int* it = neighbours_begin; it != neighbours_end; ++it)
        {
            ...
            if (!emitted_flags[triangle])
            {
                ...
                if (timestamp - cache_timestamps[a] > cache_size) cache_timestamps[a] = timestamp++;

                if (timestamp - cache_timestamps[b] > cache_size) cache_timestamps[b] = timestamp++;

                if (timestamp - cache_timestamps[c] > cache_size) cache_timestamps[c] = timestamp++;
